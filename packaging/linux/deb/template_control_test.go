...
func TestControlWrapper_SubPackages(t *testing.T) {
	...
	t.Run("WriteControl places the subpackage stanza after the primary, separated by a blank line", func(t *testing.T) {
		...
		const expectedStandardsVersion = "Standards-Version: 4.7.4\n"
		assert.Assert(t, cmp.Contains(result, expectedStandardsVersion))
		assert.Equal(t, strings.Count(result, "Standards-Version:"), 1)
		assert.Equal(t, strings.Count(result, "Source:"), 1)
		assert.Assert(t, strings.Index(result, expectedStandardsVersion) < primaryIdx)
