...
func getUserPostRequires(users []dalec.AddUserConfig, groups []dalec.AddGroupConfig) string {
	...
	if len(users) > 0 {
		// postUsersScript also invokes groupadd (to create a per-user group), so
...
// package sets Linux capabilities on an artifact that also has an ownership change
// (the case where artifactCapabilitiesScript emits a setcap call in the scriptlet).
...
func getCapabilityPostRequires(artifacts dalec.Artifacts) string {
	if artifactCapabilitiesScript(artifacts) == "" {
		return ""
	}
...
func (w *specWrapper) Requires() fmt.Stringer {
	...
	b.WriteString(getCapabilityPostRequires(artifacts))
...
func (s *subPkgWrapper) writePackageHeader(b *strings.Builder) {
	...
	// artifacts the subpackage ships (systemd units, users/groups, ownership, capabilities).
	if s.Pkg.Artifacts != nil {
		...
		b.WriteString(getCapabilityPostRequires(*s.Pkg.Artifacts))
	}
