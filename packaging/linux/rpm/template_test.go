...
func TestTemplate_Artifacts(t *testing.T) {
	t.Run("opt ownership and capabilities require setcap", func(t *testing.T) {
		t.Parallel()
		w := &specWrapper{Spec: &dalec.Spec{
...
`
		assert.Equal(t, w.Post().String(), wantPost)
		assert.Equal(t, w.Requires().String(), "Requires(post): coreutils\nRequires(post): /usr/sbin/setcap\n")
		assert.Equal(t, renderSupplementalPackage(t, w.Spec.Artifacts), `%package -n foo-supplemental
Summary: Supplemental package
Requires(post): coreutils
Requires(post): /usr/sbin/setcap

%description -n foo-supplemental
Supplemental package

%post -n foo-supplemental
chown -R myuser /opt/my-package/bin/helper
chgrp -R mygroup /opt/my-package/bin/helper
setcap 'cap_net_bind_service=ep' /opt/my-package/bin/helper

%files -n foo-supplemental
/opt/my-package/bin/helper

`)

		t.Run("capabilities without ownership use only the files macro", func(t *testing.T) {
			cfg := w.Spec.Artifacts.Opt["src/helper"]
			cfg.User, cfg.Group = "", ""
			artifacts := dalec.Artifacts{Opt: map[string]dalec.ArtifactConfig{"src/helper": cfg}}
			primary := &specWrapper{Spec: &dalec.Spec{Artifacts: artifacts}}
			assert.Equal(t, primary.Requires().String(), "")
			assert.Equal(t, primary.Post().String(), "")
			assert.Assert(t, cmp.Contains(primary.Files().String(), "%caps(cap_net_bind_service=ep) /opt/my-package/bin/helper\n"))
			supplemental := renderSupplementalPackage(t, artifacts)
			assert.Assert(t, !strings.Contains(supplemental, "Requires(post):"))
			assert.Assert(t, !strings.Contains(supplemental, "%post"))
			assert.Assert(t, cmp.Contains(supplemental, "%caps(cap_net_bind_service=ep) /opt/my-package/bin/helper\n"))
		})

		t.Run("capabilities without flags do not require setcap", func(t *testing.T) {
			cfg := w.Spec.Artifacts.Opt["src/helper"]
			cfg.LinuxCapabilities = []dalec.ArtifactCapability{{Name: "cap_net_bind_service"}}
			artifacts := dalec.Artifacts{Opt: map[string]dalec.ArtifactConfig{"src/helper": cfg}}
			primary := &specWrapper{Spec: &dalec.Spec{Artifacts: artifacts}}
			assert.Equal(t, primary.Requires().String(), "Requires(post): coreutils\n")
			assert.Assert(t, !strings.Contains(primary.Post().String(), "setcap"))
			assert.Assert(t, !strings.Contains(renderSupplementalPackage(t, artifacts), "setcap"))
		})
	})
...
func TestTemplate_TargetSpecificOverrides(t *testing.T) {
	...
	t.Run("target1 should override root values", func(t *testing.T) {
		...
		assert.Equal(t, w.Requires().String(), "")
		assert.Equal(t, w.Post().String(), "")
	})
	...
	t.Run("non-existent target should use root values", func(t *testing.T) {
		...
		assert.Equal(t, w.Requires().String(), "Requires(post): coreutils\nRequires(post): /usr/sbin/setcap\n")
		assert.Assert(t, cmp.Contains(w.Post().String(), "setcap 'cap_net_bind_service=ep' /%{_bindir}/helper\n"))
	})

	t.Run("setcap requirements stay with the owning package", func(t *testing.T) {
		for _, supplementalOwns := range []bool{false, true} {
			target := dalec.Target{
			...
			if supplementalOwns {
				target.Artifacts = &dalec.Artifacts{}
				target.Packages["helper"] = dalec.SubPackage{
					Description: "Helper",
					Artifacts:   &spec.Artifacts,
				}
			}
			s := *spec
			s.Targets = map[string]dalec.Target{"test": target}
			var out strings.Builder
			assert.NilError(t, WriteSpec(&s, "test", &out))
			got := out.String()
			primary, _, _ := strings.Cut(got, "%package ")
			assert.Equal(t, strings.Contains(primary, "Requires(post): /usr/sbin/setcap\n"), !supplementalOwns)
			assert.Equal(t, strings.Contains(primary, "setcap 'cap_net_bind_service=ep'"), !supplementalOwns)
			assert.Equal(t, strings.Count(got, "Requires(post): /usr/sbin/setcap\n"), 1)
			assert.Equal(t, strings.Count(got, "setcap 'cap_net_bind_service=ep'"), 1)
			assert.Assert(t, cmp.Contains(got, "%package -n test-pkg-meta\nSummary: Metadata only\n\n"))
			if supplementalOwns {
				assert.Assert(t, cmp.Contains(got, "%package -n test-pkg-helper\nSummary: Helper\nRequires(post): coreutils\nRequires(post): /usr/sbin/setcap\n\n"))
				assert.Assert(t, cmp.Contains(got, "%post -n test-pkg-helper\nchown -R svc /%{_bindir}/helper\nsetcap 'cap_net_bind_service=ep' /%{_bindir}/helper\n\n"))
			}
		}
	})
}
...
func TestTemplate_SupplementalPackagePreamble(t *testing.T) {
	...
	t.Run("declared users and groups provide their RPM capabilities", func(t *testing.T) {
		...
		assert.Assert(t, cmp.Contains(got, "Requires(post): /usr/sbin/groupadd, /usr/bin/getent\n"))
		assert.Assert(t, cmp.Contains(got, "getent group svcgroup >/dev/null || groupadd --system svcgroup\n"))
...
func TestTemplate_SubPackages(t *testing.T) {
	...
	t.Run("subpackage users require useradd and groupadd", func(t *testing.T) {
		t.Parallel()
		w := &specWrapper{
			Spec: &dalec.Spec{
				Name: "foo",
		...
		assert.Assert(t, cmp.Contains(s, "Requires(post): /usr/sbin/useradd, /usr/sbin/groupadd, /usr/bin/getent\n"))
		assert.Assert(t, cmp.Contains(s, "%post -n foo-service\ngetent group foouser >/dev/null || groupadd foouser\ngetent passwd foouser >/dev/null || useradd -g foouser foouser\n\n"))
	})
